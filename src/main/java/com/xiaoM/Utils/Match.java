...
public class Match {
    ...
    public static String replaceKeys(Map<String, Object> returnMap, String target) {
        ...
        for (int i = 0; i < keyNum; i++) {
            String A = "${" + keys.get(i) + "}";
            String B = "${" + keys.get(i) + "}";
            if(MainTest.commonParam.containsKey(keys.get(i))){
                B = MainTest.commonParam.get(keys.get(i));
            }
            if(returnMap.containsKey(keys.get(i))){//步骤获取的参数优先级高于公共参数
                B = returnMap.get(keys.get(i)).toString();
            }
